...
- **说人话**：使用准确名词，但不要求用户理解产品经理黑话。
- **少追问**：只问会改变最终判断的问题，信息足够就直接继续。
- **先找再造**：检索成品、近似项目和组件，而不只是传统“竞品”。
- **功能匹配**：验证实际能力，不拿名称、标签或热度代替证据。
- **承认不知道**：证据不足时写“尚未验证”，不偷偷补全结论。
- **不替用户做主**：给出明确建议，但用户始终可以继续开发。
- **控制副作用**：内部研究技能只读；只有手动主流程会保存报告。
...
这是一个早期版本。目前重点是让完整流程清楚、轻量、可运行：

- 已支持 Claude Code 插件格式；
- 已支持基础网络检索和按平台选择检索渠道；
- 尚未绑定特定爬虫、私有 API 或必须安装的 MCP 服务；
- 平台专用搜索能力和更严格的匹配评估仍会继续完善。
