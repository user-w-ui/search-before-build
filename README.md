...
`search-before-build` 不负责打击想法。它只是让你在开工前多停几分钟，用更少的成本回答三个问题：
...
### `/search-before-build:search-before-build-assess`
...
```text
/search-before-build:search-before-build-assess 我想做一个帮我自动整理收藏内容的工具
```
...
### `/search-before-build:search-before-build-compare`
...
### `search-before-build-research`（内部步骤）
...
```bash
codex plugin add search-before-build@<marketplace-name>
```

安装后请新开一个 Codex 任务，再明确请求 `$search-before-build-assess` 或 `$search-before-build-compare`。`search-before-build-research` 是两个主工作流调用的只读研究步骤，不建议直接使用。
...
│   └── search-before-build-research/SKILL.md
├── references/
