...
export function validatePipelineInput(value) {
    if (!isObject(value))
...
export function runPipeline(rawInput) {
    ...
    const normalized = input.retrievals.map((retrieval) => normalizeRetrieval(retrieval));
    ...
    const coverage = capabilities.map((capability) => {
        ...
        return {
            capability,
            status: candidateIds.length ? "candidate_mention" : "missing",
            candidateIds,
        };
    });
    ...
    const sourceCategories = [...new Set(rankResult.candidates.flatMap((candidate) => candidate.sourceCategories))];
