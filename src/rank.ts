...
const STOPWORDS = new Set(
  "a an the and or of to in on for with is are be by as at this that from how what which can should tool app system using use support plugin project 一个 一种 这个 那个 的 了 和 与 或 是 在 用 支持 工具 系统 项目 插件".split(" "),
);
...
export function tokenize(text: string): string[] {
  ...
  for (const match of normalized.matchAll(/[a-z0-9]+(?:[._-][a-z0-9]+)*/g)) {
    ...
    for (const part of token.split(/[._-]/)) {
      if (part.length > 1 && part !== token && !STOPWORDS.has(part)) tokens.push(part);
    }
  }
...
const IDENTITY_PRIORITY: Record<RecordIdentity["scheme"], number> = {
  purl: 0,
  doi: 0,
  arxiv: 0,
  mcp: 0,
...
function identityKeys(record: NormalizedRecord): string[] {
...
function textOf(record: NormalizedRecord): string {
  return [
    record.title,
    record.text.snippet,
    record.text.description,
    ...(record.text.fragments ?? []),
    ...Object.values(record.attributes).map((value) =>
...
function mergeRecords(records: NormalizedRecord[]): CandidateDraft[] {
  const groups: NormalizedRecord[][] = [];
  const identityToGroup = new Map<string, number>();
  for (const record of records) {
    if (record.status === "unusable") continue;
    const keys = identityKeys(record);
    const matchedGroups = [...new Set(keys.map((key) => identityToGroup.get(key)).filter((value): value is number => value !== undefined))];
    ...
    for (const extraIndex of matchedGroups.slice(1)) {
      ...
      for (const extraRecord of extra) {
        for (const key of identityKeys(extraRecord)) identityToGroup.set(key, groupIndex);
      }
    }
    for (const key of keys) identityToGroup.set(key, groupIndex);
  }
  return groups.filter((group) => group.length > 0).map((observations) => {
    const titles = observations.map((record) => record.title).filter((value): value is string => Boolean(value));
    const identities = new Map<string, RecordIdentity>();
    for (const record of observations) {
      for (const identity of record.identities) identities.set(`${identity.scheme}:${identity.value.toLowerCase()}`, identity);
    }
    const sourceCategories = [...new Set(observations.map((record) => record.kind).filter((value): value is SourceCategory => Boolean(value)))];
    const sortedIdentities = [...identities.values()].sort(
      (a, b) => IDENTITY_PRIORITY[a.scheme] - IDENTITY_PRIORITY[b.scheme] || a.value.localeCompare(b.value),
    );
...
function bm25f(candidates: CandidateDraft[], query: string): number[] {
  const queryTerms = [...new Set(tokenize(query))];
  if (!queryTerms.length || !candidates.length) return candidates.map(() => 0);
  ...
  for (const term of queryTerms) {
    documentFrequency.set(
      term,
      fields.filter((field) => field.title.includes(term) || field.body.includes(term)).length,
    );
  }
  const k1 = 1.2;
  const b = 0.75;
  return fields.map((field) => {
    ...
    for (const term of queryTerms) {
      ...
      if (weightedTf > 0) score += idf * ((k1 + 1) * weightedTf) / (k1 + weightedTf);
    }
    return score;
  });
}
...
function minMax(values: number[]): number[] {
  ...
  if (max === min) return values.map((value) => (value > 0 ? 1 : 0));
  return values.map((value) => (value - min) / (max - min));
}

function phraseMention(text: string, phrase: string): boolean {
...
function capabilityMention(text: string, capability: string, aliases: string[]): boolean {
...
function evidenceQuality(candidate: CandidateDraft): number {
  let score = 0;
  if (candidate.url) score += 0.35;
  if (candidate.text.length >= 80) score += 0.3;
  if (candidate.identities.some((identity) => identity.confidence === "exact")) score += 0.2;
  if (candidate.sourceCategories.some((kind) => ["repo", "package", "mcp", "paper", "page"].includes(kind))) score += 0.15;
  return Math.min(1, score);
}

// Latest updatedAt/publishedAt across merged observations, in epoch milliseconds.
function latestActivityMs(candidate: CandidateDraft): number | undefined {
...
function freshnessFromActivity(latest: number | undefined, referenceTime: number): number {
  if (latest === undefined) return 0.5;
  const ageDays = (referenceTime - latest) / 86_400_000;
  if (ageDays <= 365) return 1;
  if (ageDays >= 1095) return 0;
  return 1 - (ageDays - 365) / 730;
}

function selectDiverse(candidates: CandidateDraft[], topK: number, capabilities: string[]): CandidateDraft[] {
  ...
  while (selected.length < topK && remaining.size) {
    ...
    for (const index of remaining) {
      const candidate = candidates[index] as CandidateDraft;
    ...
    const chosen = candidates[best] as CandidateDraft;
    selected.push(chosen);
    for (const capability of chosen.matchedCapabilities) covered.add(capability);
...
export function rankCandidates(
  ...
  const rrfRaw = drafts.map((candidate) => {
    ...
    for (const observation of candidate.observations) {
      ...
      seen.add(listId);
      if (observation.provenance.providerRank) score += 1 / (60 + observation.provenance.providerRank);
    }
    return score;
  });
  const rrfScores = minMax(rrfRaw);
  drafts.forEach((candidate, index) => {
    candidate.matchedCapabilities = capabilities.filter((capability) =>
      capabilityMention(candidate.text, capability, fingerprint.capabilityAliases?.[capability] ?? []),
    );
  ...
  return {
    candidates: selected.map((candidate, index) => ({
      ...
      evidenceRefs: [...new Set(candidate.observations.flatMap((record) => [record.url, record.provenance.rawRef].filter((value): value is string => Boolean(value))))].slice(0, 10),
      features: {
      ...
      explanations: [
        ...
        latestActivityMs(candidate) !== undefined
          ? `Most recent activity ${new Date(latestActivityMs(candidate) as number).toISOString().slice(0, 10)} (freshness ${candidate.freshness.toFixed(2)}).`
