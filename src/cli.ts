...
import type { RetrievalEnvelope } from "./types.js";
...
function parseArgs(argv: string[]): CliOptions {
  ...
  const options: CliOptions = { command, compact: false };
...
async function main(): Promise<void> {
  ...
  const result = options.command === "normalize"
    ? Array.isArray(input)
      ? input.map((envelope) => normalizeRetrieval(envelope as RetrievalEnvelope))
      : normalizeRetrieval(input as RetrievalEnvelope)
    : runPipeline(input);
